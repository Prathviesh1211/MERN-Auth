  ...
  {/* Email enter form */}
    {!isEmailSent && 
      <form onSubmit={onSubmitEmail}  className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <p className='text-center mb-6 text-indigo-400'>Enter your registered email.</p>
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          <img className="size-4" src={assets.mail_icon} alt="" />
          <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)}  placeholder='Email ID'
          className='bg-transparent outline-none text-white'  />
        </div>
        <button className='w-full text-white
        mt-3 bg-gradient-to-r from-indigo-500 to-indigo-800 rounded-full py-2.5'>Sumbit</button>
      </form>
}
    ...
    {!isOtpSubmitted && isEmailSent &&
      <form onSubmit={onSubmitOtp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500
         to-indigo-800  text-white'>Submit</button>
      </form>
}
    ...
    <form onSubmit={onSubmitNewPassword}  className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          <img className="size-4" src={assets.lock_icon} alt="" />
